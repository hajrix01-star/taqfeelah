...
function buildBrandedIconSvg(size, { compact = false } = {}) {
  const taglineSize = compact
    ? Math.max(11, Math.round(size * 0.042))
    : Math.max(14, Math.round(size * 0.055));
  const taglineY = compact ? Math.round(size * 0.58) : Math.round(size * 0.64);
  return Buffer.from(`
...
async function makeIcon(size, { maskable = false, filename, includeTagline = false }) {
  const branded = includeTagline;
  const paddingRatio = maskable ? 0.2 : branded ? 0.16 : 0.14;
  const padding = Math.round(size * paddingRatio);
  const maxLogoW = size - padding * 2;
  const maxLogoH = branded
    ? Math.round(size * (maskable ? 0.28 : 0.34))
    : size - padding * 2;
  ...
  const top = branded
    ? Math.round(size * (maskable ? 0.2 : 0.24) - logoH / 2)
    : Math.round((size - logoH) / 2);

  const base = branded
    ? await sharp(buildBrandedIconSvg(size, { compact: maskable })).png().toBuffer()
    : await sharp({
      create: {
        width: size,
        height: size,
...
const jobs = [
  ...
  { size: 192, filename: "icon-maskable-192.png", maskable: true, includeTagline: true },
  { size: 384, filename: "icon-384.png", includeTagline: true },
  { size: 512, filename: "icon-512.png", includeTagline: true },
  { size: 512, filename: "icon-maskable-512.png", maskable: true, includeTagline: true },
];
