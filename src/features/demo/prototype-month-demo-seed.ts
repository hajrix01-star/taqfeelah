import type {
  ...
  DemoAttachment,
  DemoChannel,
  DemoMaterializeEntryInput,
  DemoStore,
  PrototypeMonthDemoDataset,
...
export const DEMO_ACTORS: Record<string, DemoActor> = {
...
function entryCreatedAt(iso: string, hour: number, minute = 0): string {
  const stamp = new Date(`${iso}T12:00:00`);
  stamp.setHours(hour, minute, 0, 0);
  return stamp.toISOString();
}

function demoAttachment(id: string): DemoAttachment {
  return { id, kind: "image", name: "receipt.jpg", mimeType: "image/jpeg", sizeBytes: 18400 };
}

function storeEmployee(store: DemoStore): DemoActor {
  return store.employeeId === "sara" ? DEMO_ACTORS.sara : DEMO_ACTORS.ahmed;
}

function buildSalesRecord(storeId: string, date: string): Record<string, { channelId: string; name: string; amount: number }> {
  const record: Record<string, { channelId: string; name: string; amount: number }> = {};
...
function buildOutflows(storeId: string, date: string): Array<Record<string, unknown>> {
...
function closeoutStatusFor(storeId: string, date: string): string {
...
function buildCloseout(store: DemoStore, date: string): Record<string, unknown> {
...
function materializeEntry({
  ...
  status = "active",
  extra = {},
}: DemoMaterializeEntryInput): Record<string, unknown> {
  const entryDate = String(payload.date || "");
  const salesChannels = Array.isArray(payload.salesChannels)
    ? payload.salesChannels as Array<{ amount?: number | string }>
    : [];
  const createdAt = entryCreatedAt(entryDate, hour, 10);
  const amount = payload.type === "summary"
    ? salesChannels.reduce((sum, row) => sum + Number(row.amount || 0), 0)
...
function entriesFromReviewedCloseout(closeout: Record<string, unknown>): Array<Record<string, unknown>> {
  ...
  return entries.map((item, index) => materializeEntry({
    id: `op-${String(closeout.id)}-${item.kind}-${index}`,
    payload: item.payload as Record<string, unknown>,
    actor,
    hour: item.kind === "summary" ? 22 : 11 + index,
    reviewed: item.kind !== "outflow" || seededInt(`${String(closeout.id)}|rev|${index}`, 0, 4) > 0,
    attachment: item.attachment ? demoAttachment(`att-${String(closeout.id)}-${index}`) : null,
  }));
}

function appendEdgeCaseEntries(entries: Array<Record<string, unknown>>): Array<Record<string, unknown>> {
  const extras = [
    materializeEntry({
...
function buildCloseoutEvents(closeouts: Array<Record<string, unknown>>): Array<Record<string, unknown>> {
  ...
  closeouts.forEach((closeout) => {
    ...
    events.push({
      ...
      at: entryCreatedAt(String(closeout.date), 14, 0),
      type: "opened",
...
export function createPrototypeMonthDemoDataset(): PrototypeMonthDemoDataset {
  const dates = allDemoDates();
  const closeouts: Array<Record<string, unknown>> = [];
  dates.forEach((date) => {
  ...
  let operationalEntries: Array<Record<string, unknown>> = [];
  closeouts
  ...
  return {
    closeouts: closeouts.sort((a, b) => String(b.date).localeCompare(String(a.date))),
...
export function createPrototypeMonthDemoOperationalEntries(): Array<Record<string, unknown>> {
  return createPrototypeMonthDemoDataset().operationalEntries;
}

export function countCloseoutsInMonth(closeouts: Array<{ date?: string }>, month = DEMO_MONTH): number {
  return closeouts.filter((item) => String(item.date || "").startsWith(month)).length;
}
