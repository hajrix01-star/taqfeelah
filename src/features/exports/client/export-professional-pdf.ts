...
function formatCellValue(value: unknown, column: ExportColumn, lang: DisplayLang) {
...
function sumColumn(rows: Array<Record<string, unknown>>, key: string) {
  return rows.reduce((sum, row) => sum + (Number(row[key]) || 0), 0);
}

export function exportProfessionalPdf(payload: ProfessionalExportPayload) {
