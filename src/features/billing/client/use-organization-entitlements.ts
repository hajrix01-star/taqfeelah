...
export function useOrganizationEntitlements({
  ...
  const [entitlements, setEntitlements] = useState<ResolvedOrganizationEntitlements | null>(
    () => cachedEntry?.entitlements ?? null,
  );
  ...
  const reload = useCallback(async () => {
    ...
    setError("");
    try {
      const payload = await fetchOrganizationEntitlementsViaApi({
