...
import type { DisplayCloseout } from "./employee-closeouts-types";
...
export function EmployeeCloseoutsListPanel({
  ...
  syncError: string;
  hasOlderHiddenCloseouts: boolean;
  historyScopeLabel: string;
  hiddenCloseoutCount: number;
  showStorePicker: boolean;
  assignedStores: StoreRef[];
  currentStore: StoreRef | null;
  onSelectStore?: (storeId: string) => void | Promise<void>;
  closeoutsListPending: boolean;
  displayCloseouts: DisplayCloseout[];
  sameDayCloseoutCountByDate: Map<string, number>;
  formatCalendarDate?: (date: string, lang: CloseoutSyncLang) => string;
  setCardRef: (closeoutId: string, node: HTMLElement | null) => void;
  toggleExpandedCard: (closeoutId: string) => void;
  setShareTarget: (closeout: DisplayCloseout | null) => void;
  setShareNewlySubmitted: (value: boolean | ((prev: boolean) => boolean)) => void;
  ...
  return (
    <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="taq-owner-page taq-notebook-body pb-28 pt-1">
      ...
      {showStorePicker && (assignedStores?.length || 0) > 1 && (
