...
import {
  ...
  resolveApiTotalsWithoutFinancialFallback,
...
describe("operational analytics summary helpers", () => {
  ...
  it("never substitutes local totals for API totals", () => {
    const local = { sales: 500, expense: 50, net: 450, ratio: "10.0%", proofs: 1 };
    const api = { sales: 0, expense: 0, net: 0, ratio: "0.0%", proofs: 0 };
    expect(resolveApiTotalsWithoutFinancialFallback(local, api)).toEqual(api);
    expect(resolveApiTotalsWithoutFinancialFallback(local, null)).toBeNull();
    expect(resolveApiTotalsWithoutFinancialFallback(local, { sales: 100, expense: 0, net: 100, ratio: "0.0%", proofs: 0 })).toEqual({
      sales: 100,
      expense: 0,
      net: 100,
      ratio: "0.0%",
  ...
  it("treats API zero totals as authoritative instead of using local figures", () => {
    ...
    expect(resolveOwnerPeriodSummaryPreference({ localTotals: local, apiTotals: api })).toBe(false);
    expect(resolveApiTotalsWithoutFinancialFallback(local, api)).toEqual(api);
  });

  it("uses entry-derived totals only when no API totals exist", () => {
    const local = { sales: 200, expense: 0, net: 200, ratio: "0.0%", proofs: 0 };
    const api = { sales: 150, expense: 0, net: 150, ratio: "0.0%", proofs: 0 };
    expect(resolveOwnerPeriodSummaryPreference({ localTotals: local, apiTotals: api })).toBe(false);
    expect(resolveOwnerPeriodSummaryPreference({ localTotals: local, apiTotals: null })).toBe(true);
  });

  it("keeps API totals authoritative while entries are still loading", () => {
  ...
  it("returns no trusted financial totals in DB source mode when API totals are unavailable", () => {
    const local = { sales: 900, expense: 50, net: 850, ratio: "5.6%", proofs: 3 };
    expect(resolveOwnerSingleStoreTotals(local, null, false, { entriesDbSource: true })).toBeNull();
  });
