...
import type { OwnerCloseoutManagePanelProps } from "@/features/owner-closeouts/owner-closeouts-types";

function money(value: number, lang: DisplayLang) {
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-[65] flex items-end bg-[#112A46]/40 sm:items-center sm:justify-center sm:p-6">
        <motion.div initial={{ y: 20 }} animate={{ y: 0 }} className="max-h-[90dvh] w-full overflow-y-auto rounded-t-[28px] bg-[#F8F6F0] p-5 sm:max-w-lg sm:rounded-[28px]">
          ...
          <div className="mb-4 space-y-2 rounded-2xl bg-white p-4 ring-1 ring-black/[0.045]">
            ...
            {salesRows.map((row) => (
              <div key={row.channelId} className="flex justify-between text-sm font-bold">
                <span>{row.name}</span>
                <span className="tabular-nums text-[#257844]">{money(Number(row.amount) || 0, lang)} ر.س</span>
              </div>
            ))}
            <p className="border-t border-[#F0ECE2] pt-2 text-sm font-black">{lang === "ar" ? "إجمالي الداخل" : "Total in"}: {money(Number(totals.totalSales) || 0, lang)} ر.س</p>
          </div>
          <div className="mb-4 space-y-2 rounded-2xl bg-white p-4 ring-1 ring-black/[0.045]">
            ...
            {(closeout.outflows || []).map((row) => {
              ...
              <div key={row.id} className="rounded-xl bg-[#FFFDF7] px-3 py-2 ring-1 ring-[#F0ECE2]">
                <div className="flex justify-between gap-2 text-sm font-bold">
                  ...
                  <span className="shrink-0 tabular-nums text-[#B44747]">-{money(Number(row.amount) || 0, lang)} ر.س</span>
                </div>
            ...
            <p className="border-t border-[#F0ECE2] pt-2 text-sm font-black text-[#B44747]">{lang === "ar" ? "إجمالي الخارج" : "Total out"}: {money(Number(totals.totalOutflow) || 0, lang)} ر.س</p>
          </div>
          <div className="mb-4 rounded-2xl bg-[#112A46] p-4 text-white">
            ...
            <p className="text-2xl font-black tabular-nums">{money(Number(totals.netMovement) || 0, lang)} ر.س</p>
          </div>
