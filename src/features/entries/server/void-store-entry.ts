...
const voidEntryInputSchema = z.object({
...
type VoidEntryInput = z.infer<typeof voidEntryInputSchema>;

export async function voidStoreEntry(rawInput: VoidEntryInput) {
  const parsed = voidEntryInputSchema.safeParse(rawInput);
  if (!parsed.success) {
    throw new ValidationError("Invalid void entry input.", parsed.error.flatten());
  }
  ...
  if (target.status === "voided") {
    throw new ValidationError("Entry is already voided.");
  }
