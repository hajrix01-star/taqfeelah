...
const reviewEntryInputSchema = z.object({
...
type ReviewEntryInput = z.infer<typeof reviewEntryInputSchema>;

export async function reviewStoreEntry(rawInput: ReviewEntryInput) {
  const parsed = reviewEntryInputSchema.safeParse(rawInput);
  if (!parsed.success) {
    throw new ValidationError("Invalid review entry input.", parsed.error.flatten());
  }
  ...
  if (target.status !== "active") {
    throw new ValidationError("Only active entries can be reviewed.");
  }
