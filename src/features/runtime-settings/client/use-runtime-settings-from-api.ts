...
} from "./runtime-session-and-settings-api-client";
import type { UseRuntimeSettingsFromApiProps } from "@/features/runtime-settings/client/runtime-settings-client-types";

function resolveRuntimeSettingsLoadError(lang: DisplayLang): string {
  return lang === "ar"
    ? "تعذر تحميل إعدادات التشغيل من الخادم."
    : "Failed to load runtime settings from server.";
}

function resolveRuntimeSettingsSaveError(lang: DisplayLang): string {
  return lang === "ar"
...
export function useRuntimeSettingsFromApi({
  ...
  onHydrate = () => {},
  autosaveDelayMs = 450,
}: UseRuntimeSettingsFromApiProps = {}) {
  ...
  const persistNow = useCallback(async (partialSettings: Record<string, unknown> = {}) => {
    ...
    fetchRuntimeSettingsViaApi(auth)
      .then((payload) => {
        ...
        if (payload?.settings && typeof payload.settings === "object") {
          onHydrate(payload.settings as Record<string, unknown>);
