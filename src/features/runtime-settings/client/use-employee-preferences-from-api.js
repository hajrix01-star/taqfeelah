...
} from "./employee-preferences-api-client.js";

function resolveEmployeePreferencesLoadError(lang) {
  return lang === "ar"
    ? "تعذر تحميل إعداداتك من الخادم."
    : "Failed to load your preferences from the server.";
}

function resolveEmployeePreferencesSaveError(lang) {
  return lang === "ar"
...
export function useEmployeePreferencesFromApi({
  ...
  const persistNotebookTheme = useCallback(async (notebookTheme) => {
