...
function readShareAmount(value: unknown): number {
  const parsed = Number(value);
  return Number.isFinite(parsed) ? parsed : 0;
}

/** Accept UI totals, API ack payloads, and persisted closeout totals. */
export function normalizeCloseoutShareTotals(
  totals: CloseoutShareTotalsInput | null | undefined,
): CloseoutShareTotals | null {
  ...
  const net = netFromFields != null && String(netFromFields) !== ""
...
function outflowRowLabel(item: CloseoutOutflow, lang: CloseoutSyncLang): string {
...
function formatOpTime(iso: string | null | undefined, lang: CloseoutSyncLang): string {
  if (!iso) return "";
