...
import type { ShareImageResult } from "./daily-closeouts-types";

function openWhatsAppWithText(message: string): void {
  if (typeof window === "undefined") return;
  openWhatsAppShare(message || "", resolveSupportWhatsAppNumber());
}

export async function copyShareCaptionText(caption: string): Promise<boolean> {
...
type SystemSharePayload = ShareData & { files?: File[] };

export async function shareImageThroughSystem({
  file,
  caption = "",
  title = "",
  ...
  const captionPayload: SystemSharePayload = {
    files: [file],
    ...(caption ? { text: caption } : {}),
    ...(title ? { title } : {}),
  };
  const payloads: SystemSharePayload[] = caption
    ? [captionPayload]
    : [
      captionPayload,
  ...
  for (const payload of payloads) {
    try {
      ...
      if (error instanceof Error && error.name === "AbortError") return { ok: true, method: "abort" };
    }
  }
  return { ok: false, method: "unsupported" };
}
