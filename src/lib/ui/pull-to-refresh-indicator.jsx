...
/**
 * Maps pull progress to visible indicator scale (native-style fade-in).
 * @param {number} pullProgress 0..1
 * @param {boolean} refreshing
 */
export function resolvePullToRefreshIndicatorScale(pullProgress, refreshing) {
  if (refreshing) return 1;
  return 0.35 + Math.min(1, pullProgress) * 0.65;
}

/**
 * Maps pull progress to arc length on the progress ring.
 * @param {number} pullProgress 0..1
 * @param {boolean} refreshing
 */
export function resolvePullToRefreshArcOffset(pullProgress, refreshing) {
