...
import { AlertTriangle, Check, Info, Trash2, X, type LucideIcon } from "lucide-react";
import type { AppActionSheetProps, AppDialogVariant } from "@/lib/ui/app-dialog/app-dialog-types";

const VARIANT_STYLES: Record<AppDialogVariant, {
  iconWrap: string;
  icon: LucideIcon;
  confirm: string;
}> = {
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          {notice ? (
            <p className="mt-2 text-taq-meta font-bold leading-6 text-[#716753]">{notice}</p>
          ) : null}
          {children}
          {mode === "confirm" ? (
