...
type PrototypeRuntimeLoggedOutGateProps = {
  lang: PrototypeLang;
  ...
  onOwnerLogin: PrototypeRuntimeCallback;
  onEmployeeLogin: PrototypeRuntimeCallback;
};
...
export function PrototypeRuntimeLoggedOutGate({
  ...
  onOwnerLogin,
  onEmployeeLogin,
}: PrototypeRuntimeLoggedOutGateProps) {
  return (
...
export function PrototypeRuntimeOrgLoadingGate({ lang }: { lang: PrototypeLang }) {
  return (
...
type PrototypeRuntimeOwnerPasswordGateProps = {
  lang: PrototypeLang;
  setLang: (lang: PrototypeLang) => void;
  onComplete: () => void;
  onLogout: () => void;
...
export function PrototypeRuntimeOwnerPasswordGate({
  ...
  onComplete,
  onLogout,
}: PrototypeRuntimeOwnerPasswordGateProps) {
  return (
    <div dir={lang === "ar" ? "rtl" : "ltr"} className="taq-app-root min-h-[100dvh] bg-[#F8F6F0] font-sans text-[#112A46]">
      <AppFontStyles />
      <OwnerPasswordChangeScreen
...
export function PrototypeRuntimeOrgErrorGate({
  lang,
  orgConfigSyncError,
}: {
  lang: PrototypeLang;
