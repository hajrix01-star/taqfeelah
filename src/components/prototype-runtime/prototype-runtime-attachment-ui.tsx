...
import type { EntryAttachmentApiContext, OperationalEntry, PrototypeLang } from "./prototype-runtime-types";
...
export function useAttachmentCapture(lang: PrototypeLang) {
  ...
  const selectAttachment = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setError(text(lang, message === "invalid" ? "invalidAttachment" : "attachmentTooLarge"));
    } finally {
      setProcessing(false);
    }
  };
...
export function mergeAttachmentApiContext(
...
export function useAttachmentSource(
  ...
  attachmentApiContext: EntryAttachmentApiContext = null,
  storeId = "",
) {
