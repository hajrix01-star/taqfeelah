...
import {
  ...
  sanitizeAmountInput,
  toAmount,
} from "./prototype-runtime-entry-form-utils";
import {
  businesses,
  businessLocation,
  businessName,
...
export function OwnerSummaryScreen({
  ...
  const submit = () => canSave && !processing && !saving && onSave({
    ...
    businessId,
    type: "summary",
    salesChannels: salesChannels.map((channel) => ({ channelId: channel.id, name: channelName(channel, lang), amount: toAmount(values[channel.id]) })).filter((row) => row.amount > 0),
    attachment,
  ...
  return (
    <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mx-auto w-full pb-24 sm:max-w-[560px] lg:max-w-none">
      ...
      <div className="space-y-5 px-5">
        ...
        <div className="grid grid-cols-2 gap-3"><SmallInfo label={text(lang, "date")} value={formatCalendarDate(operationDate, lang)} /><SmallInfo label={text(lang, "category")} value={categoryLabel} /></div>
