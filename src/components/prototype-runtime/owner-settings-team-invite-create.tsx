...
import type { OwnerSettingsTeamInviteCreateProps } from "./prototype-runtime-types";
...
export function OwnerSettingsTeamInviteCreate({
  ...
  displayBusinessName,
  invites,
}: OwnerSettingsTeamInviteCreateProps) {
  ...
  return (
    <div className="mb-4 rounded-3xl bg-white p-4 ring-1 ring-black/[0.045]">
      ...
      {invites.createdInvite ? (() => {
        const createdInvite = invites.createdInvite;
        return (
        <div className="mt-4 rounded-2xl bg-[#FFF8E8] p-3">
          ...
          <p dir="ltr" className="mt-1 text-xs font-black">
            {formatDisplayDateTime(createdInvite.expiresAt ?? "", lang)}
          </p>
          <div className="mt-3 flex flex-wrap gap-2">
            <button type="button" onClick={() => { void invites.copyText(createdInvite.inviteUrl ?? "", "link"); }} className="rounded-full bg-white px-3 py-2 text-taq-meta font-black">{invites.copiedField === "link" ? labels.copied : labels.copyLink}</button>
            <button type="button" onClick={() => { void invites.copyText(createdInvite.pin ?? "", "pin"); }} className="rounded-full bg-white px-3 py-2 text-taq-meta font-black">{invites.copiedField === "pin" ? labels.copied : labels.copyPin}</button>
            <button type="button" onClick={() => invites.shareInviteWhatsApp(createdInvite)} className="rounded-full bg-[#25D366] px-3 py-2 text-taq-meta font-black text-white">{labels.shareWhatsApp}</button>
            <button type="button" onClick={() => { void invites.revokeInvite(createdInvite.invitationId ?? ""); }} className="rounded-full bg-[#FFF1EE] px-3 py-2 text-taq-meta font-black text-[#B44747]">{labels.revoke}</button>
          </div>
        </div>
        );
      })() : null}
