...
import type { OwnerSettingsTeamPendingInvitesProps } from "./prototype-runtime-types";
...
export function OwnerSettingsTeamPendingInvites({
  ...
  loading,
  onRevokeInvite,
}: OwnerSettingsTeamPendingInvitesProps) {
          ...
          <div key={invite.invitationId} className="rounded-2xl bg-[#F7F5EF] p-3">
            <div className="flex items-center justify-between gap-2">
              ...
              <span className="rounded-full bg-white px-2 py-1 text-taq-meta font-bold">
                {formatInviteStatus(String(invite.status ?? ""), lang)}
              </span>
            </div>
            {invite.phoneNumber ? (
              <p className="mt-1 text-taq-meta font-bold text-[#716753]" dir="ltr">
                {labels.phoneLabel}: {formatLoginPhoneForDisplay(String(invite.phoneNumber))}
              </p>
            ) : null}
            <p className="mt-1 text-taq-meta font-bold text-[#827762]">{invite.storeName}</p>
            <button
              type="button"
              onClick={() => { void onRevokeInvite(String(invite.invitationId)); }}
