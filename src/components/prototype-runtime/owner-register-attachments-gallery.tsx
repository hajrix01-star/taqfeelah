...
import type { DisplayLang, EntryAttachmentApiContext, PrototypeBusiness } from "./prototype-runtime-types";
...
function GalleryTile({
  ...
  return (
    <article className="min-w-0">
      ...
      <button
        type="button"
        onClick={() => item.entry && onOpenOperation(item.entry)}
...
function LazyGallerySection({ children, rootMargin = "320px" }: { children: React.ReactNode; rootMargin?: string }) {
...
type AttachmentGalleryDay = {
  date: string;
  dateLabel?: string;
  items: Array<Record<string, unknown> & { id?: string }>;
};

type AttachmentGallerySection = {
  id: string;
  heading?: string;
  days: AttachmentGalleryDay[];
};
...
export function OwnerRegisterAttachmentsGallery({
  ...
  apiRegisterEntriesHasMore?: boolean;
                  ...
                  <div className="grid grid-cols-3 gap-3 sm:grid-cols-4">
                    {day.items.map((item) => (
                      <GalleryTile
                        ...
                        daySequenceByCloseoutId={daySequenceByCloseoutId ?? new Map()}
                        sameDayCloseoutCountByStoreDate={sameDayCloseoutCountByStoreDate ?? new Map()}
                      />
                    ))}
                  </div>
                </div>
