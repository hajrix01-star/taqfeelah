...
function NetValue({ lang, value }: { lang: DisplayLang; value: unknown }) {
  ...
  return (
    <strong className={`text-center tabular-nums ${positive ? "text-[var(--taq-color-257844)]" : "text-[var(--taq-color-b44747)]"}`}>
...
export function OwnerRegisterGeneralReportList({
  ...
  if (loading) {
    return (
      <div className="rounded-[18px] bg-white px-4 py-8 text-center text-taq-meta font-bold text-[var(--taq-color-806528)] ring-1 ring-[var(--taq-color-e8e1d4)]">
        {text(lang, "loading")}
      </div>
    );
  }
  ...
  return (
    <article className="overflow-hidden rounded-[18px] border border-[var(--taq-color-e8e1d4)]/90 bg-white shadow-[0_2px_4px_rgba(17,42,70,0.04),0_8px_20px_rgba(17,42,70,0.06)]">
      <div className="border-b border-[var(--taq-color-f0ebe0)] px-3 py-2">
        <div className={`${GRID_CLASS} text-taq-nav font-bold text-[var(--taq-color-806528)]`}>
      ...
      <div className="px-3 py-1">
        {rows.map((row, index) => (
          <div
            ...
            className={`${GRID_CLASS} min-h-[44px] py-2 text-taq-meta ${index < rows.length - 1 ? "border-b border-[var(--taq-color-f0ebe0)]" : ""}`}
          >
            <span
              className={`block w-full truncate font-bold text-[var(--taq-color-112a46)] ${dayColumnAlignClass(lang)}`}
              dir="ltr"
            >
              {formatRegisterReportRowLabel(String(row.date ?? ""), granularity as RegisterReportGranularity, lang)}
            </span>
            <strong className="text-center tabular-nums text-[var(--taq-color-257844)]">
              <MoneyValue value={money(Number(row.sales ?? 0), lang)} />
            </strong>
            <strong className="text-center tabular-nums text-[var(--taq-color-b44747)]">
      ...
      <div className="border-t-2 border-[var(--taq-color-112a46)]/15 bg-[var(--taq-color-f6f8fb)] px-3 py-2">
        <div className={`${GRID_CLASS} min-h-[44px] text-taq-meta font-bold`}>
          <span className={`text-[var(--taq-color-112a46)] ${dayColumnAlignClass(lang)}`}>{text(lang, "combinedTotal")}</span>
          <strong className="text-center tabular-nums text-[var(--taq-color-257844)]">
          ...
          <div className="text-center">
            <strong className={`tabular-nums ${netPositive ? "text-[var(--taq-color-257844)]" : "text-[var(--taq-color-b44747)]"}`}>
              <MoneyValue value={money(totals.net, lang)} />
            </strong>
            {netMarginRatio !== "—" ? (
              <span className="mt-0.5 block text-[9px] font-bold text-[var(--taq-color-827762)]">
