...
function SubscriptionUsageMeter({ label, used, max, lang }: { label: React.ReactNode; used: number; max: number; lang: DisplayLang }) {
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-taq-meta font-bold text-[var(--taq-color-716753)]">
        <span>{label}</span>
        <span className={overLimit ? "text-[var(--taq-color-b44747)]" : ""}>{used} / {max}</span>
      </div>
      <div className="h-2 overflow-hidden rounded-full bg-[var(--taq-color-f0ece2)]">
        <div
          className={`h-full rounded-full transition-all ${overLimit ? "bg-[var(--taq-color-b44747)]" : "bg-[var(--taq-color-112a46)]"}`}
          style={{ width: `${percent}%` }}
        />
      </div>
      {overLimit ? (
        <p className="text-taq-meta font-bold text-[var(--taq-color-b44747)]">
...
export function OwnerSettingsSubscriptionSection({
  ...
  return (
    <SettingsSectionFrame embedded={embedded}>
      ...
      {entitlementsError ? (
        <div className="mb-4 rounded-3xl bg-[var(--taq-color-fff1ee)] p-4 text-center ring-1 ring-black/[0.045]">
          <p className="text-taq-meta font-bold text-[var(--taq-color-b44747)]">{entitlementsError}</p>
          <button type="button" onClick={() => { void reloadEntitlements(); }} className="mt-3 rounded-2xl bg-[var(--taq-color-112a46)] px-4 py-2.5 text-taq-meta font-black text-white">
            {text(lang, "retryLoad")}
          </button>
        </div>
      ) : null}
      {entitlements ? (
        <>
          {entitlements.isTrialPlan ? (
            <div className="mb-4 rounded-3xl bg-[var(--taq-color-fff4d2)] p-5 ring-1 ring-[var(--taq-color-f0d9a2)]">
              <Badge tone="warning">{text(lang, "trialPlanBadge")}</Badge>
              <p className="mt-3 text-sm font-black text-[var(--taq-color-806528)]">{text(lang, "trialPlanTitle")}</p>
              <p className="mt-3 text-taq-meta font-bold text-[var(--taq-color-806528)]">
          ...
          <div className="mb-4 rounded-3xl bg-white p-5 ring-1 ring-black/[0.045]">
            ...
            {Number.isInteger(entitlements.accountNumber) ? (
              <p className="mt-2 text-taq-meta font-bold text-[var(--taq-color-827762)]" dir="ltr">
                {lang === "ar" ? "رقم الحساب: " : "Account no.: "}
                {formatOrganizationAccountNumber(entitlements.accountNumber)}
              </p>
            ) : null}
            <p className="mt-2 text-taq-meta font-bold leading-6 text-[var(--taq-color-716753)]">
            ...
            <p className="mt-2 text-taq-meta font-bold text-[var(--taq-color-827762)]">
            ...
            <p className="mt-3 text-taq-meta font-bold text-[var(--taq-color-827762)]">
              {entitlements.isTrialPlan
          ...
          <div className="mb-4 rounded-3xl bg-white p-5 ring-1 ring-black/[0.045]">
            <p className="text-xs font-bold text-[var(--taq-color-716753)]">{text(lang, "planFeatures")}</p>
            <ul className="mt-3 space-y-2">
              {entitlements.features.map((feature) => (
                <li key={feature.key} className="rounded-2xl bg-[var(--taq-color-f7f5ef)] px-4 py-3 text-taq-meta font-bold text-[var(--taq-color-112a46)]">
          ...
          {entitlements.upgradePlans.length && !hideUpgradeActions ? (
            <div className="rounded-3xl bg-white p-5 ring-1 ring-black/[0.045]">
              <p className="text-xs font-bold text-[var(--taq-color-716753)]">{text(lang, "upgradeOptions")}</p>
              <div className="mt-3 space-y-3">
                {entitlements.upgradePlans.map((plan) => (
                  <div key={plan.planCode} className="rounded-2xl border border-[var(--taq-color-f0ece2)] p-4">
                    <div>
                    ...
                    <ul className="mt-3 space-y-1.5">
                      {plan.features.map((feature) => (
                        <li key={`${plan.planCode}-${feature.key}`} className="text-taq-meta font-bold text-[var(--taq-color-716753)]">
                          • {pickLocalizedFeatureLabel(feature, lang)}
                        </li>
                      ))}
                    </ul>
          ...
          {!hideUpgradeActions && !entitlements.upgradePlans.length ? (
            <div className="rounded-3xl bg-white p-5 ring-1 ring-black/[0.045]">
              <button type="button" onClick={onOpenSupport} className="w-full rounded-2xl bg-[var(--taq-color-112a46)] py-3 text-xs font-black text-white">
