  ...
  return (
    <SettingsSectionFrame embedded={embedded}>
      ...
      <div className="mb-3 flex items-center justify-between">
        <p className="text-xs font-bold text-[var(--taq-color-716753)]">{text(lang, "activeStores")}</p>
        <button onClick={() => setShowAddStore(!showAddStore)} className="flex items-center gap-1 text-taq-meta font-black text-[var(--taq-color-9a823e)]"><Plus className="h-3.5 w-3.5" />{text(lang, "addStore")}</button>
      </div>
      {showAddStore && (
        <div className="mb-4 rounded-3xl bg-white p-4 ring-1 ring-black/[0.045]">
          {atStoreLimit ? (
            <p className="mb-4 rounded-2xl bg-[var(--taq-color-fff1ee)] p-3 text-center text-taq-meta font-bold leading-6 text-[var(--taq-color-b44747)]">
              {storeLimitMessage}
            </p>
          ) : null}
          <input value={newStoreName} onChange={(event) => setNewStoreName(event.target.value)} placeholder={text(lang, "newStoreName")} disabled={atStoreLimit} className="mb-2 w-full rounded-2xl bg-[var(--taq-color-f7f5ef)] px-4 py-3 text-xs font-bold outline-none disabled:opacity-60" />
          <input value={newStoreLocation} onChange={(event) => setNewStoreLocation(event.target.value)} placeholder={text(lang, "newStoreLocation")} disabled={atStoreLimit} className="mb-4 w-full rounded-2xl bg-[var(--taq-color-f7f5ef)] px-4 py-3 text-xs font-bold outline-none disabled:opacity-60" />
          <button type="button" onClick={() => { void addStore(); }} disabled={storeSaving || atStoreLimit} className="w-full rounded-2xl bg-[var(--taq-color-112a46)] py-3 text-xs font-black text-white disabled:opacity-60">
      ...
      {(settingsNotice || settingsSuccess) ? (
        <div className="mb-4">
          {settingsNotice ? (
            <p className="rounded-xl bg-[var(--taq-color-fff1ee)] p-2.5 text-center text-taq-meta font-bold text-[var(--taq-color-b44747)]">{settingsNotice}</p>
          ) : null}
          {settingsSuccess ? (
            <div className="mt-2 rounded-xl bg-[var(--taq-color-e6f5e9)] p-3 text-center text-taq-meta font-black text-[var(--taq-color-257844)]">{text(lang, "changesSaved")}</div>
          ) : null}
        </div>
      ) : null}
      <div className="mb-4 overflow-hidden rounded-3xl bg-white ring-1 ring-black/[0.045]">
        {activeStoredBusinesses.length ? activeStoredBusinesses.map((business, index) => (
          <button key={business.id} onClick={() => openStore(business.id)} className={`flex w-full items-center justify-between px-4 py-4 text-start ${index < activeStoredBusinesses.length - 1 ? "border-b border-[var(--taq-color-f0ece2)]" : ""}`}>
            <div>
              ...
              <p className="mt-1 text-taq-meta font-bold text-[var(--taq-color-827762)]">
                ...
                <span className="text-[var(--taq-color-257844)]">{text(lang, "storeActive")}</span>
                {typeof countEmployeesForStore === "function" ? (
                  <span className="text-[var(--taq-color-716753)]">
                    {" · "}
            ...
            <Arrow className="h-4 w-4 text-[var(--taq-color-b99844)]" />
          </button>
        )) : <p className="p-5 text-center text-xs font-bold text-[var(--taq-color-827762)]">{text(lang, "noActiveStores")}</p>}
      </div>
      {archivedStoredBusinesses.length > 0 && (
        <>
          <button onClick={() => setShowArchivedStores(!showArchivedStores)} className="mb-3 flex items-center gap-1 text-taq-meta font-black text-[var(--taq-color-9a823e)]">
            ...
            <div className="overflow-hidden rounded-3xl bg-white ring-1 ring-black/[0.045]">
              {archivedStoredBusinesses.map((business) => (
                <button key={business.id} onClick={() => openStore(business.id)} className="flex w-full items-center justify-between px-4 py-4 text-start opacity-70">
                  <div>
                    <p className="text-xs font-black">{displayBusinessName(business)}</p>
                    <p className="mt-1 text-taq-meta font-bold text-[var(--taq-color-b96725)]">{text(lang, "archivedStore")}</p>
                  </div>
