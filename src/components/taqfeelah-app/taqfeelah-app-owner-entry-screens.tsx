...
function EntryDatePicker({ lang, value, onChange, showSuggestion = false }: {
  ...
  return (
    <div ref={pickerRef} className="relative mb-5">
      <div className="mb-2 flex items-center justify-between gap-2"><p className="text-xs font-bold text-[var(--taq-color-716753)]">{text(lang, "date")}</p>{showSuggestion && <span className="rounded-full bg-[var(--taq-color-fff0cb)] px-2 py-1 text-taq-nav font-bold text-[var(--taq-color-806528)]">{text(lang, "suggestedNextCloseout")}</span>}</div>
      <button type="button" onClick={() => { setCalendarView({ year: selected.getFullYear(), month: selected.getMonth() }); setOpen(!open); }} className="flex w-full items-center justify-between rounded-2xl bg-white px-4 py-3.5 text-sm font-black text-[var(--taq-color-112a46)] ring-1 ring-black/[0.05]">
        <span>{formatCalendarDate(value, lang)}</span><CalendarDays className="h-4 w-4 text-[var(--taq-color-b99844)]" />
      </button>
      {showSuggestion && <p className="mt-2 text-taq-meta font-bold text-[var(--taq-color-827762)]">{text(lang, "changeDateAnytime")}</p>}
      <AnimatePresence>{open && <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} className="absolute start-0 end-0 top-[78px] z-30 rounded-2xl bg-[var(--taq-color-fffdf7)] p-3 shadow-xl ring-1 ring-[var(--taq-color-d8cca8)]">
        <div className="mb-3 flex items-center justify-between"><button type="button" onClick={previous} className="flex h-8 w-8 items-center justify-center rounded-xl text-[var(--taq-color-806528)]"><ChevronRight className={`h-4 w-4 ${lang === "en" ? "rotate-180" : ""}`} /></button><strong className="text-xs">{formatCalendarMonth(calendarView.year, calendarView.month, lang)}</strong><button type="button" onClick={next} className="flex h-8 w-8 items-center justify-center rounded-xl text-[var(--taq-color-806528)]"><ChevronLeft className={`h-4 w-4 ${lang === "en" ? "rotate-180" : ""}`} /></button></div>
        <div className="mb-2 grid grid-cols-7 text-center text-taq-meta font-bold text-[var(--taq-color-957d43)]">{weekDays.map((day, index) => <span key={`${day}-${index}`}>{day}</span>)}</div>
        <div className="grid grid-cols-7 gap-1 text-center text-xs font-bold">{dates.map((date) => date.day ? <button key={date.key} type="button" disabled={Boolean(date.iso && date.iso > todayLimit)} onClick={() => { if (date.iso && date.iso <= todayLimit) { onChange(date.iso); setOpen(false); } }} className={`flex h-8 items-center justify-center rounded-lg ${date.iso && date.iso > todayLimit ? "cursor-not-allowed text-[var(--taq-color-c8c0b1)]" : date.iso === value ? "bg-[var(--taq-color-b44747)] text-white" : "text-[var(--taq-color-112a46)] hover:bg-[var(--taq-color-fff0cb)]"}`}>{date.day}</button> : <span key={date.key} className="h-8" />)}</div>
...
function Choice({ active, children, onClick }: { active: boolean; children: ReactNode; onClick: () => void }) {
  return <button type="button" onClick={onClick} className={`rounded-2xl py-3 text-xs font-extrabold ${active ? "bg-[var(--taq-color-112a46)] text-white" : "bg-white text-[var(--taq-color-716753)] ring-1 ring-black/[0.05]"}`}>{children}</button>;
}

function SmallInfo({ label, value }: { label: ReactNode; value: ReactNode }) {
  return <div className="rounded-2xl bg-white p-3 ring-1 ring-black/[0.05]"><p className="text-taq-meta font-bold text-[var(--taq-color-716753)]">{label}</p><p className="mt-1 text-xs font-black">{value}</p></div>;
}
...
function StoreOperationPicker({ lang, businessesList = businesses, selectedId, onSelect }: {
  ...
  if (!searchable) {
    return <div className="grid grid-cols-2 gap-2">{businessesList.map((business) => <button key={business.id} type="button" onClick={() => onSelect(business.id)} className={`rounded-2xl px-3 py-3 text-xs font-black ${selectedId === business.id ? "bg-[var(--taq-color-112a46)] text-white" : "bg-[var(--taq-color-f7f5ef)] text-[var(--taq-color-716753)] ring-1 ring-black/[0.05]"}`}>{businessName(business, lang, true) || businessName(business, lang)}</button>)}</div>;
  }
  return (
    <div ref={pickerRef} className="relative">
      <button type="button" onClick={() => setOpen(!open)} className="flex w-full items-center justify-between rounded-2xl bg-white px-4 py-3 text-start text-xs font-black ring-1 ring-black/[0.05]"><span>{selectedStore ? businessName(selectedStore, lang) : text(lang, "selectStore")}</span><ChevronDown className="h-4 w-4 text-[var(--taq-color-806528)]" /></button>
      <AnimatePresence>{open && <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} className="absolute start-0 end-0 top-[50px] z-40 rounded-2xl bg-white p-3 shadow-xl ring-1 ring-[var(--taq-color-e8e1d4)]"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={text(lang, "searchStore")} className="mb-2 w-full rounded-xl bg-[var(--taq-color-f7f5ef)] px-3 py-2.5 text-taq-meta font-bold outline-none" /><div className="max-h-48 overflow-y-auto">{filteredStores.map((business) => <button key={business.id} type="button" onClick={() => { onSelect(business.id); setOpen(false); setQuery(""); }} className={`mb-1 flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-start ${selectedId === business.id ? "bg-[var(--taq-color-fff4d2)]" : ""}`}><div><p className="text-taq-meta font-black">{businessName(business, lang)}</p><p className="text-taq-nav font-bold text-[var(--taq-color-827762)]">{businessLocation(business, lang)}</p></div>{selectedId === business.id && <Check className="h-4 w-4 text-[var(--taq-color-112a46)]" />}</button>)}</div></motion.div>}</AnimatePresence>
    </div>
  );
}
  ...
  return (
    <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mx-auto w-full pb-24 sm:max-w-[560px]">
      ...
      <div className="space-y-5 taq-page-gutter">
        {showStorePicker ? (
          <div>
            <p className="mb-2 text-xs font-bold text-[var(--taq-color-716753)]">{text(lang, "operationStore")}</p>
            <StoreOperationPicker lang={lang} businessesList={businessesList} selectedId={businessId} onSelect={changeStore} />
            <p className={`mt-2 text-taq-meta font-bold ${selectedStore ? "text-[var(--taq-color-827762)]" : "text-[var(--taq-color-b44747)]"}`}>
        ...
        {!showStorePicker ? (
          <div>
            <p className="mb-2 text-xs font-bold text-[var(--taq-color-716753)]">{text(lang, "operationStore")}</p>
            <StoreOperationPicker lang={lang} businessesList={businessesList} selectedId={businessId} onSelect={changeStore} />
            <p className={`mt-2 text-taq-meta font-bold ${selectedStore ? "text-[var(--taq-color-827762)]" : "text-[var(--taq-color-b44747)]"}`}>{!selectedStore ? text(lang, "chooseStoreForSummary") : null}</p>
          </div>
        ) : null}
        <div className="rounded-3xl bg-white p-4 ring-1 ring-black/[0.05]">
          <p className="mb-3 text-xs font-bold text-[var(--taq-color-716753)]">{text(lang, "salesChannels")}</p>
          {salesChannels.length === 0 ? <div className="rounded-2xl bg-[var(--taq-color-fff1ee)] p-5 text-xs font-bold text-[var(--taq-color-b44747)]">{text(lang, "noSalesChannels")}</div> : <div className="grid grid-cols-3 gap-2">{salesChannels.map((channel) => <label key={String(channel.id)} className="rounded-2xl bg-[var(--taq-color-f7f5ef)] px-2 py-3 text-center ring-1 ring-black/[0.05]"><span className="mb-2 block min-h-[30px] text-taq-meta font-bold leading-4 text-[var(--taq-color-716753)]">{channelName(channel, lang)}</span><div dir="ltr" className="flex items-center justify-center gap-1"><input inputMode="decimal" value={values[String(channel.id)] || ""} onChange={(event) => setValues((current) => ({ ...current, [String(channel.id)]: sanitizeAmountInput(event.target.value) }))} className="min-w-0 w-full bg-white px-1 py-2 text-center text-sm font-black outline-none" /><span className="text-taq-nav font-bold text-[var(--taq-color-827762)]">{lang === "ar" ? "ر.س" : "SAR"}</span></div></label>)}</div>}
          <div className="mt-4 rounded-2xl bg-[var(--taq-color-112a46)] p-4 text-white">
            ...
            {total > 0 ? (
              <div className="mt-3 border-t border-white/15 pt-3">
                ...
                {error ? <p className="mt-2 text-taq-nav font-black text-[var(--taq-color-ffb4b4)]">{error}</p> : null}
                <ProofAttachmentPreview
                  lang={lang}
        ...
        <button type="button" disabled={!canSave || processing || saving} onClick={submit} className={`w-full rounded-2xl py-4 text-sm font-extrabold text-white ${canSave && !processing && !saving ? "bg-[var(--taq-color-39a160)]" : "bg-[var(--taq-color-b8c0b7)]"}`}>{text(lang, saving ? "saving" : "save")}</button>
        </fieldset>
      </div>
    </motion.section>
  );
      ...
      <div className="space-y-5 taq-page-gutter">
        ...
        {!showStorePicker ? (
          <div>
            <p className="mb-2 text-xs font-bold text-[var(--taq-color-716753)]">{text(lang, "operationStore")}</p>
            <StoreOperationPicker lang={lang} businessesList={businessesList} selectedId={businessId} onSelect={changeStore} />
            <p className={`mt-2 text-taq-meta font-bold ${selectedStore ? "text-[var(--taq-color-827762)]" : "text-[var(--taq-color-b44747)]"}`}>{!selectedStore ? text(lang, "chooseOperationStore") : null}</p>
          </div>
        ) : null}
        <div>
          <p className="mb-2 text-xs font-bold text-[var(--taq-color-716753)]">{text(lang, "transactionType")}</p>
        ...
        {kind === "expense" && <div><p className="mb-2 text-xs font-bold text-[var(--taq-color-716753)]">{text(lang, "category")}</p>{activeCategories.length ? <div className="grid grid-cols-3 gap-2">{activeCategories.map((item) => <Choice key={item.id} active={category === item.id} onClick={() => setCategory(item.id)}>{text(lang, item.label)}</Choice>)}</div> : <p className="rounded-xl bg-[var(--taq-color-fff1ee)] p-3 text-taq-meta font-bold text-[var(--taq-color-b44747)]">{text(lang, "atLeastOneCategory")}</p>}</div>}
        <div className="rounded-3xl bg-white p-5 ring-1 ring-black/[0.05]">
          <p className="text-xs font-bold text-[var(--taq-color-716753)]">{text(lang, "amount")}</p>
          <div className="mt-2 flex items-center gap-2" dir="ltr">
            ...
            <span className="mt-3 text-sm font-bold text-[var(--taq-color-786d58)]">{lang === "ar" ? "ر.س" : "SAR"}</span>
          </div>
          {toAmount(amount) > 0 ? (
            <div className="mt-4 border-t border-[var(--taq-color-e8e1d4)] pt-3">
              <p className="text-taq-nav font-bold text-[var(--taq-color-827762)]">{text(lang, "captureAttachment")} ({text(lang, "optional")})</p>
              {!resolveAttachmentPreviewSrc(attachment) ? (
                <ProofAddButton lang={lang} onSelect={selectAttachment} disabled={processing} processing={processing} className="mt-2" />
              ) : null}
              {error ? <p className="mt-2 text-taq-nav font-black text-[var(--taq-color-b44747)]">{error}</p> : null}
              <ProofAttachmentPreview
                lang={lang}
                src={resolveAttachmentPreviewSrc(attachment)}
                onRemove={clearAttachment}
        ...
        <div className="rounded-3xl bg-white p-4 ring-1 ring-black/[0.05]"><p className="mb-2 text-xs font-bold text-[var(--taq-color-716753)]">{text(lang, "note")} <span className="font-normal">({text(lang, "optional")})</span></p><textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder={text(lang, "notePlaceholder")} className="min-h-[52px] w-full resize-none rounded-2xl bg-[var(--taq-color-f7f5ef)] px-4 py-3 text-sm outline-none" /></div>
        <button type="button" disabled={!canSave || processing || saving} onClick={submit} className={`w-full rounded-2xl py-4 text-sm font-extrabold text-white transition ${canSave && !processing && !saving ? "bg-[var(--taq-color-112a46)]" : "cursor-not-allowed bg-[var(--taq-color-b8c0b7)]"}`}>{text(lang, saving ? "saving" : "saveOutflow")}</button>
        </fieldset>
      </div>
    </motion.section>
  );
