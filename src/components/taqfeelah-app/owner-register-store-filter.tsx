  ...
  if (businessesList.length <= 1) {
    ...
    return (
      <NotebookRow className="justify-center">
        <p className="text-xs font-black text-[var(--taq-color-806528)]">{businessName(businessesList[0], lang, true) || businessName(businessesList[0], lang)}</p>
      </NotebookRow>
    );
  }
  const stores = locked
      ...
      <NotebookRow>
        <div className={`grid w-full items-end gap-2 ${stores.length === 2 ? "grid-cols-2" : "grid-cols-3"}`}>
          {stores.map((store) => {
            ...
            return (
              <button key={store.id} type="button" disabled={locked} onClick={() => setSelectedBusiness(store.id)} className={`relative min-w-0 pb-2 text-center text-xs font-black transition ${active ? "text-[var(--taq-color-b44747)]" : "text-[var(--taq-color-957d43)]"} ${locked ? "cursor-default" : ""}`}>
                <span className="relative inline-flex whitespace-nowrap">{store.label}{active && <span className="absolute -bottom-[9px] left-0 right-0 h-[2px] rounded-full bg-[var(--taq-color-c28a30)]" />}</span>
              </button>
            );
          })}
        </div>
  ...
  return (
    <NotebookRow className="justify-center">
      <div ref={filterRef} className="relative pb-[8px]">
        <button type="button" onClick={() => setOpen(!open)} className={`inline-flex max-w-[238px] items-center justify-center gap-1.5 rounded-full px-3 py-1 text-taq-meta font-bold transition ${open ? "text-[var(--taq-color-b44747)]" : "text-[var(--taq-color-806528)]"}`}>
          <span className="truncate">{selectedBusiness === "all" ? text(lang, "allStores") : businessName(selectedStore, lang)}</span>
          <ChevronDown className={`h-3.5 w-3.5 shrink-0 transition ${open ? "rotate-180 text-[var(--taq-color-b44747)]" : "text-[var(--taq-color-806528)]"}`} />
        </button>
        <AnimatePresence>{open && <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} className="absolute left-1/2 top-[38px] z-40 w-[270px] -translate-x-1/2 rounded-2xl bg-[var(--taq-color-fffdf7)] p-3 shadow-xl ring-1 ring-[var(--taq-color-d8cca8)]">
          ...
          <button type="button" onClick={() => { setSelectedBusiness("all"); setOpen(false); setQuery(""); }} className={`mb-1 flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-xs font-bold ${selectedBusiness === "all" ? "bg-[var(--taq-color-fff0cb)] text-[var(--taq-color-b44747)]" : "text-[var(--taq-color-112a46)]"}`}><span>{text(lang, "allStores")}</span>{selectedBusiness === "all" && <Check className="h-4 w-4" />}</button>
  ...
  if (businessesList.length <= 1) {
    ...
    return (
      <div className="mb-2 flex justify-center">
        <span className="rounded-full bg-[var(--taq-color-112a46)]/[0.06] px-3 py-1 text-[11px] font-black text-[var(--taq-color-112a46)]">
          ...
          return (
            <button
              ...
              className={`rounded-full px-3 py-1.5 text-[11px] font-black transition-colors duration-150 ${
                active
                  ? "bg-[var(--taq-color-112a46)] text-white shadow-[0_2px_8px_rgba(17,42,70,0.18)]"
                  : "bg-white text-[var(--taq-color-716753)] ring-1 ring-[var(--taq-color-e8e1d4)]"
