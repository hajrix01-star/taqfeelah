...
export function OwnerSettingsHomeSection({
  ...
  return (
    <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="taq-page-gutter pb-24">
      <div className="mb-5">
        <p className="text-xs font-bold text-[var(--taq-color-8b8274)]">{text(lang, "ownerAccount")}</p>
      ...
      <button onClick={() => setSection("account")} className="mb-5 flex w-full items-center gap-4 rounded-3xl bg-white p-4 text-start ring-1 ring-black/[0.045]">
        <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[var(--taq-color-112a46)] text-white"><UserRound className="h-6 w-6" /></div>
        <div className="min-w-0 flex-1">
          <p className="text-sm font-black">{String(ownerProfile?.name || (lang === "ar" ? "المالك" : "Owner"))}</p>
          <p className="mt-1 text-taq-meta font-bold text-[var(--taq-color-827762)]">{text(lang, "myAccountSecurity")}</p>
        </div>
        <Arrow className="h-4 w-4 shrink-0 text-[var(--taq-color-b99844)]" />
      </button>
      <p className="mb-2 text-xs font-bold text-[var(--taq-color-716753)]">{lang === "ar" ? "المنشأة" : "Organization"}</p>
      ...
      <p className="mb-2 text-xs font-bold text-[var(--taq-color-716753)]">{lang === "ar" ? "التفضيلات" : "Preferences"}</p>
      <div className="mb-5 overflow-hidden rounded-3xl bg-white ring-1 ring-black/[0.045]">
      ...
      <p className="mb-2 text-xs font-bold text-[var(--taq-color-716753)]">{text(lang, "support")}</p>
