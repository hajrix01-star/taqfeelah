...
import copy from "./taqfeelah-app-copy";
import type {
  AppBusiness,
  AppChannel,
  AppExpenseCategory,
...
const channelCatalog: Record<string, AppChannel> = {
...
const channelName: (channel: AppChannel | Record<string, unknown>, lang: AppLang) => string = (channel, lang) => resolveSalesChannelLabel(channel, lang, text);

const expenseCategories: AppExpenseCategory[] = [
...
const emptyStoreRecord: AppStoreRecord = { sales: 0, expense: 0, ratio: "0.0%", net: 0, proofs: 0 };
...
const businessName = (business: AppBusiness | null | undefined, lang: AppLang, short = false) => {
  if (!business) return "";
...
const businessLocation = (business: AppBusiness | null | undefined, lang: AppLang) => business?.locationKey ? text(lang, business.locationKey) : (String(business?.customLocation || ""));
const businessRecord = (business: AppBusiness | null | undefined, monthly: boolean) => (monthly ? business?.month : business?.day) || emptyStoreRecord;
const combinedTotals = (monthly: boolean, storeList: AppBusiness[] = businesses) => storeList.reduce<{ sales: number; expense: number; net: number; proofs: number }>((total, business) => {
...
const text: AppTextFn = (lang, key) => copy[lang][key as keyof typeof copy.ar] || key;
const money: AppMoneyFn = (value, lang) => formatDisplayMoneyLabel(value, lang);
const fullDate = (day: { fullAr?: string; fullEn?: string }, lang: AppLang) => lang === "ar" ? String(day.fullAr || "") : String(day.fullEn || "");
const shortDate = (day: { dayAr?: string; dayEn?: string }, lang: AppLang) => lang === "ar" ? String(day.dayAr || "") : String(day.dayEn || "");
const opDate = (item: { date?: string; dateAr?: string; dateEn?: string }, lang: AppLang) => item.date ? formatCalendarDate(item.date, lang) : (lang === "ar" ? String(item.dateAr || "") : String(item.dateEn || ""));
const opTime = (item: { createdAt?: string; timeAr?: string; timeEn?: string }, lang: AppLang) => item.createdAt ? new Intl.DateTimeFormat(lang === "ar" ? "ar-SA-u-nu-latn" : "en-US", { hour: "numeric", minute: "2-digit" }).format(new Date(item.createdAt)) : (lang === "ar" ? String(item.timeAr || "") : String(item.timeEn || ""));
const auditDateTime = (timestamp: string, lang: AppLang) => {
