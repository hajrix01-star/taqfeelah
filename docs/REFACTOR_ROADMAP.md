...
| 2026-06-14 | **دفعة احترافية P6–P8** — PIN من `auth_identities`، إيقاف `provisionStaffMembers` مع org-config API، فرض حدود الباقة داخل المعاملات |
...
| P6 | PIN من `auth_identities` فقط | 🟠 | PR قيد المراجعة |
| P7 | إيقاف `provisionStaffMembers` في الإنتاج | 🟠 | PR قيد المراجعة |
| P8 | قيود DB على حدود الباقة | 🟠 | PR قيد المراجعة |
