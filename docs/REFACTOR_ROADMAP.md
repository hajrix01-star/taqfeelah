...
> آخر تحديث: **2026-06-12** — دفعة تنظيف ما قبل الإطلاق (`docs/PRELAUNCH_CLEANUP.md`)؛ موجات 1–7 مفعّلة.
...
| 2026-06-14 | **دفعة UX باقات (قيد التنفيذ)** — رسالة واضحة عند رفض إضافة محل (حد الخطة)؛ تمييز «أساسية + فترة تجريبية» عن «خطة تجربة مجانية»؛ PIN SSOT؛ إزالة legacy blob |
...
| P3 | رسالة واضحة عند رفض إضافة محل (حد الخطة) | 🟠 | المنطق يعمل؛ الواجهة كانت صامتة |
| P4 | تمييز «أساسية + فترة تجريبية» عن «تجربة مجانية» | 🟠 | شارة «تجريبي» كانت تُربك المالك |
| P5 | تنظيف بيانات فوق الحد (`db:reconcile-entitlements`) | 🟠 | سكربت جاهز — تشغيل على VPS |
| P6 | PIN من `auth_identities` فقط | 🟠 | لم يبدأ |
| P7 | إيقاف `provisionStaffMembers` في الإنتاج | 🟠 | لم يبدأ |
| P8 | قيود DB على حدود الباقة | 🟠 | لم يبدأ |
