...
ينطبق المنع أيضًا على التقفيلات: التعديل يلغي العمليات السابقة ويحفظها، والحذف إلغاء منطقي للتقفيلة وعملياتها. لا تستخدم `delete(entries)` أو `delete(dailyCloseouts)` في مسارات المنتج.
...
- تم حذف `/prototype-runtime`. مسار التشغيل المعتمد هو `/app` فقط، ولا يوجد مسار مرجعي منفصل للتطبيق.
...
**مسار التطبيق المعتمد:** `/app`. الموقع العام منفذ على `/`، ولا يوجد مسار prototype منفصل.
...
| `/app` | تطبيق تقفيلة التشغيلي المعتمد |
...
| Landing vs App مباشر | إنتاج: `/` تسويق + `/app` تشغيل؛ لا يوجد مسار prototype منفصل — §10 |
...
| تعديل بعد الإرسال | **المالك/المدير فقط** عبر `mode: "ownerEdit"`؛ النسخة السابقة تصبح `voided` ولا تُحذف |
