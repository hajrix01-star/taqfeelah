...
مراجعة المبيعات/إثبات الصور للمالك (`reviewEnabled` في إعدادات المحل، **افتراضيًا معطّلة**).

### سلوك الاعتماد التلقائي (مقصود — لا يُغيّر بدون قرار منتج)

| `closeoutReviewEnabled` | السلوك |
|-------------------------|--------|
| **معطّل (الافتراضي)** | إرسال الموظف = اعتماد تلقائي: الحالة `reviewed`، السجل التشغيلي `active`، **بدون** تنبيه «بانتظار المراجعة» للمالك. |
| **مفعّل** | الإرسال = `submitted` حتى يعتمد المالك أو يُرجع التقفيلة. |

الواجهة ترسل `autoReview: true` للـ API عند إغلاق المراجعة. السيرفر يجب أن يُكمل `closeout_approved` ويُفعّل الـ entries في نفس المعاملة — بما في ذلك دور **الموظف**.
